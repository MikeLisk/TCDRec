...
import pandas as pd
import numpy as np
from  TCDRec import  TCDRec, collate, collate_test
...
from  TCDRec_utils import eval_metric, mkdir_if_not_exist, Logger

class TestCollateWrapper:
    """用于在 Windows 多进程中传递 data_neg 的包装类，防止序列化报错"""
    def __init__(self, data_neg):
        self.data_neg = data_neg
        
    def __call__(self, batch):
        return collate_test(batch, self.data_neg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--user_long', default='dg_stgn')
    parser.add_argument('--item_long', default='dg_stgn')
    ...
    parser.add_argument('--layer_num', type=int, default=4, help='GNN layer')
    ...
    if opt.record:
        log_file = f'results/{opt.data}_ba_{opt.batchSize}_G_{opt.gpu}_dim_{opt.hidden_size}_ulong_{opt.user_long}_ilong_{opt.item_long}_' \
                   f'_La_{args.last_item}_UM_{opt.user_max_length}_IM_{opt.item_max_length}_K_{opt.k_hop}' \
    ...
    test_collate_fn = TestCollateWrapper(data_neg)
    ...
    if opt.val:
        val_data = DataLoader(dataset=val_set, batch_size=opt.batchSize, collate_fn=test_collate_fn, pin_memory=True, num_workers=4)

    model =  TCDRec(user_num=user_num, item_num=item_num, input_dim=opt.hidden_size, item_max_length=opt.item_max_length,
                 user_max_length=opt.user_max_length, feat_drop=opt.feat_drop, attn_drop=opt.attn_drop, user_long=opt.user_long, 
                 item_long=opt.item_long, user_update=opt.user_update, item_update=opt.item_update, last_item=opt.last_item,
                 layer_num=opt.layer_num, tcan_enabled=True).to(device)
    ...
    for epoch in range(opt.epoch):
        ...
        model.train()
        for user_batch, batch_graph, label, last_item in train_data:
            iter_num += 1
            score, tcan_loss = model(batch_graph.to(device), user_batch.to(device), last_item.to(device), is_training=True)
            loss = loss_func(score, label.to(device))
            
            alpha = 0.05 
            total_loss = loss + alpha * tcan_loss
            
            optimizer.zero_grad()
            total_loss.backward()
            
            # 【核心修复】：梯度裁剪，防止偶尔的极端梯度冲破网络导致 NaN
            ...
            if iter_num % 400 == 0:
                print('Iter {}, loss {:.4f}, main_loss {:.4f}, tcan_loss {:.4f}'.format(
                    iter_num, epoch_loss/iter_num, loss.item(), tcan_loss.item()), datetime.datetime.now())
        epoch_loss /= iter_num
        model.eval()
        ...
        if opt.val:
            ...
            with torch.no_grad():
                for user_batch, batch_graph, label, last_item, neg_tar in val_data:
                    score, top = model(batch_graph.to(device), user_batch.to(device), last_item.to(device), neg_tar=torch.cat([label.unsqueeze(1), neg_tar], -1).to(device), is_training=False)
        ...
        all_loss = []
        with torch.no_grad():
            for user_batch, batch_graph, label, last_item, neg_tar in test_data:
                iter_num += 1
                score, top = model(batch_graph.to(device), user_batch.to(device), last_item.to(device), neg_tar=torch.cat([label.unsqueeze(1), neg_tar],-1).to(device),  is_training=False)
                ...
                if iter_num % 200 == 0:
                    print('Iter {}, test_loss {:.4f}'.format(iter_num, np.mean(all_loss)), datetime.datetime.now())
            ...
            if recall10 > best_result[1]:
                if opt.model_record:
                    mkdir_if_not_exist('save_models')
